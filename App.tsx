...
const App: React.FC = () => {
  ...
  const closeQuiz = () => {
    if (quizFeedback === 'CORRECT') {
      ...
      if (!canRevive.current) setGameState('GAMEOVER');
  ...
  const gameLoop = useCallback(() => {
    ...
    particles.current.forEach(p => { ctx.globalAlpha = p.life; ctx.fillRect(p.x, p.y, 4, 4); });
    ctx.globalAlpha = 1.0;
    const dx = 50;
  ...
  }, [gameState, theme, highScore, isLoadingTheme, isInvincible]);
    ...
    <div className="flex flex-col items-center min-h-screen py-16 px-4 relative overflow-x-hidden">
      <div className="text-center mb-16 animate-in slide-in-from-top duration-1000 w-full px-4 flex flex-col items-center">
        <div className="glass px-20 py-14 rounded-[5rem] shadow-[0_40px_80px_-20px_rgba(0,0,0,0.1)] border border-white/50 relative overflow-hidden group mb-10">
          ...
          <h1 className="flex flex-col items-center relative z-10">
            <span className="text-7xl md:text-[10rem] font-pixel text-emerald-900 drop-shadow-[0_10px_10px_rgba(0,0,0,0.1)] italic tracking-tighter leading-none animate-glow-emerald">T-REX</span>
            <span className="text-3xl md:text-5xl font-pixel text-amber-500 mt-6 drop-shadow-lg tracking-widest uppercase animate-glow-amber">KECEPIRIT</span>
          </h1>
        </div>
        <div className="glass px-10 py-4 rounded-full shadow-2xl border border-white/80 animate-in fade-in duration-1000 delay-300">
           <p className="text-emerald-700 font-black tracking-[0.4em] text-[10px] md:text-xs uppercase italic">
             ...
             {isInvincible && (
               <div className="flex flex-col items-start gap-2 animate-in slide-in-from-left duration-500">
                 <div className="px-1 flex flex-col gap-1.5">
                   <span className="text-[7px] font-black text-amber-600 uppercase font-pixel tracking-[0.2em] drop-shadow-sm ml-1">Angel Mode Active</span>
                   <div className="relative w-56 h-3.5 bg-slate-900/40 rounded-full border border-white/30 overflow-hidden shadow-[0_0_25px_rgba(251,191,36,0.15)] backdrop-blur-xl">
          ...
          {gameState === 'PLAYING' && (
            <div className="absolute top-10 right-10 flex gap-5 pointer-events-none z-[60] animate-in fade-in duration-500">
              <div className="glass px-7 py-4 rounded-[2.5rem] shadow-2xl border border-white/70 flex flex-col items-end">
                <span className="text-[8px] text-slate-400 font-black uppercase tracking-widest mb-1">RECORD</span>
                <span className="text-slate-900 font-black text-2xl font-pixel">{highScore.toString().padStart(5, '0')}</span>
              </div>
              <div className="bg-emerald-800/90 backdrop-blur-xl px-7 py-4 rounded-[2.5rem] shadow-2xl border border-emerald-600/30 flex flex-col items-end">
                <span className="text-[8px] text-emerald-200 font-black uppercase tracking-widest mb-1">SCORE</span>
                <span className="text-white font-black text-2xl font-pixel">{score.toString().padStart(5, '0')}</span>
              </div>
            </div>
          )}

          {(gameState === 'START' || gameState === 'GAMEOVER') && (
            <div className="absolute inset-0 bg-slate-900/20 backdrop-blur-sm flex items-center justify-center p-8 z-50 animate-in fade-in duration-700">
                <div className="bg-white/95 backdrop-blur-3xl px-10 py-12 rounded-[3.5rem] shadow-[0_40px_100px_rgba(0,0,0,0.4)] border border-white flex flex-col items-center w-[360px] max-w-full text-center transform border-t-[12px] border-t-emerald-600">
                  <div className="text-8xl mb-8 animate-bounce leading-none">{gameState === 'START' ? '🦖' : '💥'}</div>
                  <h2 className="text-2xl font-black text-slate-900 mb-2 uppercase font-pixel tracking-tighter leading-tight">
                    {gameState === 'START' ? 'Siap Lari?' : 'Kecepirit!'}
                  </h2>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-10 px-4">
                    {gameState === 'START' ? 'Ayo hindari rintangan kaktus' : `Skor yang diraih: ${score} meter`}
                  </p>
                  <button onClick={resetGame} className="w-full bg-emerald-700 hover:bg-emerald-800 text-white font-black py-6 rounded-[2.5rem] text-2xl shadow-2xl transition-all transform active:scale-95 uppercase font-pixel tracking-tighter">
           ...
           <div className="bg-white/95 backdrop-blur-2xl w-full max-w-lg rounded-[2.5rem] shadow-[0_60px_120px_-30px_rgba(0,0,0,0.5)] border border-white/60 flex flex-col items-center p-6 md:p-8 text-center relative animate-in zoom-in-95 duration-500 overflow-y-auto max-h-[95vh]">
              ...
              {currentQuiz && (
                <div className="w-full space-y-3 md:space-y-4">
                  {!quizExplanation ? (
                    <>
                      <div className="bg-slate-50/80 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border border-white shadow-inner">
                        <p className="text-sm md:text-base font-bold text-slate-800 leading-relaxed italic">"{currentQuiz.question}"</p>
                      </div>
                      <div className={`grid grid-cols-1 gap-2 md:gap-3 ${quizFeedback !== 'NONE' ? 'pointer-events-none' : ''}`}>
                        {currentQuiz.options.map((opt, i) => (
                          <button 
                            ...
                            className={`flex items-center justify-center w-full font-bold py-3 md:py-4 px-4 md:px-6 rounded-[1.2rem] md:rounded-[1.5rem] border border-white shadow-md transition-all active:scale-95 text-[10px] md:text-xs italic ${
                              selectedIndex === i 
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
                      <div className="bg-slate-100/90 backdrop-blur-sm p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border border-white shadow-inner text-left">
                        <h4 className="text-[8px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2 font-pixel">Dino's Wisdom</h4>
                        {isExplaining ? (
                          <div className="flex justify-center py-2"><div className="w-6 h-6 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin"></div></div>
                        ) : (
                          <p className="text-xs md:text-sm font-bold text-slate-700 leading-relaxed italic">"{quizExplanation}"</p>
                        )}
                      </div>
                      <button onClick={closeQuiz} className="w-full bg-slate-900 hover:bg-black text-white font-black py-4 md:py-5 rounded-[1.2rem] md:rounded-[1.5rem] text-base md:text-lg shadow-xl transition-all active:scale-95 tracking-widest font-pixel">CONTINUE</button>
                    </div>
                  )}
                </div>
              )}
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-3 gap-8 mb-16 px-4">
        <div className="glass p-10 rounded-[3.5rem] shadow-xl border border-white/50 text-center flex flex-col items-center hover:translate-y-[-5px] transition-transform">
          ...
          <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 font-pixel">Lompat</h3>
          <p className="text-xs font-bold text-slate-600 leading-relaxed italic">Gunakan Spasi atau Panah Atas untuk melompati kaktus.</p>
        </div>
        <div className="glass p-10 rounded-[3.5rem] shadow-xl border border-white/50 text-center flex flex-col items-center hover:translate-y-[-5px] transition-transform">
          <div className="w-16 h-16 bg-amber-50 text-amber-600 rounded-[1.5rem] flex items-center justify-center mb-6 text-3xl shadow-inner border border-amber-100">👼</div>
          <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 font-pixel">Kebal</h3>
          <p className="text-xs font-bold text-slate-600 leading-relaxed italic">Dapatkan perlindungan 'Angel Mode' dengan menjawab kuis secara benar.</p>
        </div>
        <div className="glass p-10 rounded-[3.5rem] shadow-xl border border-white/50 text-center flex flex-col items-center hover:translate-y-[-5px] transition-transform">
          <div className="w-16 h-16 bg-blue-50 text-blue-700 rounded-[1.5rem] flex items-center justify-center mb-6 text-3xl shadow-inner border border-blue-100">🎨</div>
          <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 font-pixel">AI Theme</h3>
          <p className="text-xs font-bold text-slate-600 leading-relaxed italic">Ketik perintah di chat untuk mengganti <b>warna Dino, cuaca, atau suasana siang/malam</b> secara instan.</p>
        </div>
      </div>
      ...
      <div className="w-full max-w-5xl glass rounded-[5rem] shadow-[0_80px_150px_-50px_rgba(0,0,0,0.2)] border-[14px] border-white overflow-hidden flex flex-col md:flex-row h-[600px] mb-20 relative">
        <div className="md:w-1/3 bg-slate-900 p-14 text-white flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="w-24 h-24 bg-white/10 rounded-[3rem] flex items-center justify-center text-5xl mb-12 backdrop-blur-xl border border-white/20">🦖</div>
            <h2 className="text-5xl font-black uppercase italic mb-6 leading-none tracking-tighter">Dino <br/><span className="text-emerald-400">Genius</span></h2>
            <p className="text-slate-400 text-sm font-bold leading-relaxed opacity-80 italic">
              "Mau warna badan baru atau suasana malam hari? Bilang aja, nanti aku sulap!"
            </p>
          </div>
          <div className="relative z-10 glass-dark px-8 py-4 rounded-[2rem] text-[9px] font-black tracking-widest text-emerald-300 uppercase shadow-xl">REAL-TIME ENV CONTROL</div>
        </div>
          ...
          <div className="flex-1 p-12 overflow-y-auto space-y-8 scroll-smooth">
            {chatMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-4 duration-500`}>
                <div className={`max-w-[85%] px-10 py-6 rounded-[3rem] text-sm font-bold shadow-lg ${
            ...
            {isChatLoading && (
              <div className="flex justify-start"><div className="glass px-8 py-4 rounded-full flex gap-3 shadow-md"><div className="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-bounce"></div><div className="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-bounce delay-100"></div><div className="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-bounce delay-200"></div></div></div>
            )}
          </div>
          <div className="p-10 glass-dark border-t border-white/10 flex gap-5 items-center">
            <input type="text" value={chatInput} onFocus={() => { isTyping.current = true; }} onBlur={() => { isTyping.current = false; }} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSendMessage()} placeholder="Coba: 'Ganti dino jadi ungu' atau 'Buat jadi malam'..." className="flex-1 bg-white/10 border-none rounded-[2.5rem] px-10 py-6 text-sm font-bold text-white placeholder-slate-500 focus:ring-4 focus:ring-emerald-500/50 outline-none transition-all shadow-inner" />
            <button onClick={handleSendMessage} disabled={isChatLoading} className="bg-emerald-500 hover:bg-emerald-400 active:scale-95 text-slate-900 h-20 px-12 rounded-[2.5rem] font-black transition-all disabled:opacity-50 shadow-[0_15px_30px_rgba(16,185,129,0.3)] uppercase tracking-widest text-xs">SEND</button>
          </div>
        </div>
      </div>
      ...
      <footer className="mb-20 text-slate-400 text-[9px] md:text-[10px] font-black uppercase tracking-[0.5em] flex items-center gap-12 w-full max-w-5xl px-8 text-center flex-col md:flex-row">
        ...
        <div className="flex flex-col gap-2">
          <span className="text-slate-500 tracking-[0.3em]">Bahasa Inggris &bull; DOSEN: Marisa Fran Lina M.Pd</span>
          <span className="text-emerald-500/60 tracking-[0.2em]">Created by Azis, Arkan & Oryza</span>
        </div>
        <div className="hidden md:block h-px flex-1 bg-slate-200/60" />
      </footer>
    </div>
